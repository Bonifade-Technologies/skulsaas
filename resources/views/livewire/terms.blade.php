...
<div class="w-full">
    <div class="grid grid-cols-1 space-y-4">
        <div class="space-y-4">
            <x-bread-crumb :inp="['name' => 'Terms', 'url' => 'academics/terms']" />
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
                {{-- <x-search /> --}}
                <div wire:submit.prevent="addSession" class="flex flex-col items-end gap-2 md:flex-row">
    ...
    <div class="flex flex-col w-full overflow-x-auto border divide-x divide-gray-300 md:flex-row">
                ...
                @if ($allSessions->count())
                    @forelse ($years as $year)
                        <li wire:click="setYear({{ $year->id }})"
                            class="relative flex items-center justify-between px-3 py-2 border border-gray-300 rounded cursor-pointer tt dark:border-gray-800 hover:bg-slate-200'
                    {{ $year_id == $year->id ? 'bg-slate-100  shadow border-0 font-medium' : 'bg-white dark:bg-slate-700 dark:text-white' }}">
                        ...
                        class="absolute text-xl text-red-600 rounded cursor-pointer -top-1 -left-1 "
        ...
        <div class="flex-1 overflow-x-auto">
            @if ($allSessions->count())
                <form wire:submit.prevent=@if ($update) 'updateTerm' @else 'addTerm' @endif
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @forelse ($currentSessionTerms as $term)
                                <tr class="even:bg-gray-100 dark:even:bg-gray-700">
                                    <td
                                        class="px-4 py-2 space-x-2 text-base text-gray-900 capitalize whitespace-nowrap dark:text-white">
                                    ...
                                    <td class="flex items-center p-2 space-x-2 whitespace-nowrap">
            ...
            @else
                <div class="flex items-center justify-center h-full text-center align-middle py-auto">No term
