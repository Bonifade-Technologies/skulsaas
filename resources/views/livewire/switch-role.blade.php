<div class="z-20 z-50 hidden max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:bg-gray-700 dark:divide-gray-600"
    ...
    <div class="grid grid-cols-3 gap-4 p-4">
        ...
        <button wire:click="switchRole({{ $role->id }})"
            class="block p-4 text-center rounded-lg {{ $role->id == auth()->user()->current_role_id ? 'bg-gray-50':'' }} shadow relative hover:text-white hover:bg-primary dark:hover:bg-gray-600">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-8 h-8 text-gray-500 mb-1 mx-auto p-1 rounded bg-gray-100">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
            </svg>
            <div class="text-sm capitalize font-medium dark:text-white">
                {{ $role->name }}
            </div>
            @if ($role->id == auth()->user()->current_role_id)
            <span class="absolute top-0 right-0 m-1 text-green-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
